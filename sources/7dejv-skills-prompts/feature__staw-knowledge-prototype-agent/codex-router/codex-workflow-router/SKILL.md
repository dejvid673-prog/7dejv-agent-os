...
description: Main 7dejv work-mode router for Codex. Use when a task needs choosing the right skills, procedures, agent references, command procedures, or 7dejv workflow for PrestaShop, PHP modules, SQL/VAT reports, Allegro, BaseLinker, ecommerce operations, SEO/content, architecture decisions, pre-commit review, deployment checks, long tasks, knowledge bases, visual prototypes, or handoff. Routes to 1-3 skills, optionally one agent procedure or persona, and requires planning before larger code/database/API/configuration changes plus verification after changes.
---
...
### H. Knowledge base, research and visual prototype

Use for a knowledge base, help center, documentation portal, encyclopedia, research repository, clickable HTML/CSS mockup, information architecture, UX research, content cards, visual-first project review, and projects where the user needs to see progress from the first iteration.

Default 7DEJV agent:

- `7dejv/agents/7dejv-staw-knowledge-prototype-agent.md` for Repetytorium Wiedzy STAW EXPERT.
...
Candidate skills: choose 1-3 depending on the current slice:

- research/structure: `ux-researcher-designer`, `knowledge-ops`, `content-strategy`, `product-research` or `deep-research`,
- visual prototype: `frontend-design`, `ui-design-system`, `senior-frontend`,
- QA: `playwright-cli` or `playwright-pro`, `a11y-audit`, `full-page-screenshot`, `S003`,
- defects: `S002` only when a concrete error exists.

Procedure:

- `agent-procedures/engineering/cs-frontend-engineer.md` for prototype and rendering,
- `7dejv/agents/7dejv-pond-product-expert.md` may be used as a domain reference, not as the main noncommercial knowledge-base agent.

Rules:

1. Inspect the real project, source map, topic register and current prototype before editing.
2. Select a bounded slice of 1-3 topics.
3. Develop research and the clickable visualization in the same iteration.
4. A user-facing structural change cannot finish as Markdown only.
5. Use real topic names rather than `Lorem ipsum`.
6. Mark unverified content visibly; appearance is not evidence.
7. Render changed screens in a real browser and store screenshots.
8. Check local links, assets, console errors and horizontal overflow.
9. Do not build a production backend or PrestaShop module unless the task is explicitly handed off.
10. After a long iteration, update research status, QA evidence and handoff.
